...
async function loadCharacters(charactersArg) {
  let characterPaths = charactersArg?.split(",").map((path) => path.trim()).map((path) => `./${path}`);
  const loadedCharacters = [];
  if (characterPaths?.length > 0) {
    for (const path of characterPaths) {
      try {
        const character2 = JSON.parse(fs.readFileSync(path, "utf8"));
        if (character2.plugins) {
          console.log("Plugins are: ", character2.plugins);
          const importedPlugins = await Promise.all(
            character2.plugins.map(async (plugin) => {
              const importedPlugin = await import(plugin);
              return importedPlugin;
            })
          );
          character2.plugins = importedPlugins;
        }
...
async function createAgent(character2, db, token) {
  elizaLogger.success(
...
async function startAgent(character2, directClient) {
  try {
    ...
    const runtime = await createAgent(character2, db, token);
